<script>
  ...
  function getPieceImage(piece) {
    ...
    return (
      PIECE_MAP[key] ||
      `${PIECE_CDN}/${piece.color === "w" ? "w" : "b"}${piece.type.toUpperCase()}.png`
    );
  }
  ...
  function handleDragStart(e, square) {
    ...
    if (roomState.isOnline && !roomState.isMyTurn) {
      e.preventDefault();
      return;
    }
...
<div class="board-container">
  <div
    class="chess-board"
    style:transform={roomState.orientation === "black"
      ? "rotate(180deg)"
      : "rotate(0deg)"}
  >
    {#each displayRanks as rank}
      {#each displayFiles as file}
        ...
        <div
          ...
          data-square={square}
          style={portal
            ? `--portal-color: ${portal.color}; --portal-charge: ${gameState.portalCharge.toFixed(1)};`
            : ""}
...
<style>
  ...
  /* Last move */
  .square.last-move {
    background-color: rgba(232, 255, 99, 0.884) !important;
  }
