<script>
  ...
  onMount(async () => {
    ...
    if (session) {
      ...
      try {
        ...
        const canRejoin =
          data.currentUserRole === session.role &&
          data.currentUserRole !== "none";

        if (canRejoin) {
          discoveredSession = { ...session, data };
        } else {
          console.warn(
            "[Lobby] Role mismatch or room full. Stored:",
            session.role,
            "Server:",
  ...
  async function handleJoin() {
    ...
    if (!code) {
      joinError = "Enter a code.";
      return;
    }
    if (!/^[A-Z0-9]{4,8}$/.test(code)) {
      joinError = "Invalid code format.";
      return;
    }
...
<div class="lobby-menu" style="animation: introReveal 0.4s ease forwards;">
    ...
    <div class="rejoin-card">
      <div class="rejoin-info">
        ...
        <div class="rejoin-text">
          ...
          <span class="rejoin-subtitle"
            >Room {discoveredSession.code} • {discoveredSession.role.toUpperCase()}</span
          >
        </div>
      </div>
      <div class="rejoin-actions">
        <button class="btn text-btn sm" onclick={handleDismissSession}
          >Dismiss</button
        >
  ...
  <div class="lobby-section">
    <h3>Host a Game</h3>
    <button
      class="btn filled main-btn"
      onclick={handleCreate}
      disabled={waiting}
    >
      {waiting ? "Waiting..." : "Generate Room"}
    </button>
